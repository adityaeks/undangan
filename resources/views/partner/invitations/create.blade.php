<x-partner-layout>
    <div 
        ...
        x-data="{ 
            ...
            errorMessage: '',
            selectedTheme: '{{ old('theme_id', $themes->first()?->id ?? '') }}',
    ...
    >
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-2 border-b border-sand-200">
            <div>
                <div class="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-brand-500/10 text-brand-800 text-[10px] font-bold uppercase tracking-wider border border-brand-500/20 mb-1">
                    ...
                    <span>Portal Partner &amp; WO • Buat Undangan Klien</span>
                </div>
                <h1 class="font-serif text-xl sm:text-2xl font-bold text-charcoal-950">
                    Buat Undangan Klien Baru
                </h1>
                <p class="text-xs text-sand-600">
                    Lengkapi data pernikahan klien, foto cover, galeri prewedding, jadwal acara, dan rekening amplop digital.
                </p>
            </div>

            <a href="{{ route('partner.invitations.index') }}" class="self-start sm:self-auto inline-flex items-center gap-1.5 px-3.5 py-2 rounded-xl bg-sand-200 hover:bg-sand-300 text-charcoal-900 text-xs font-bold transition">
        ...
        <!-- FORM WRAPPER -->
        <form method="POST" action="{{ route('partner.invitations.store') }}" enctype="multipart/form-data" @submit="submitForm($event)" novalidate class="space-y-4">
            @csrf
            ...
            <div class="p-5 sm:p-6 rounded-2xl glass-panel border border-sand-200 space-y-5">
                ...
                <div x-show="currentStep === 1" data-step="1" class="space-y-4">
                    <div class="flex items-center justify-between pb-2 border-b border-sand-200">
                        <div>
                            ...
                            <p class="text-[11px] text-sand-600">Pilih klien pemesan dan tentukan tema desain super admin yang ingin digunakan.</p>
                        </div>
                        ...
                        <div class="sm:col-span-2">
                            ...
                            <div class="flex items-center">
                                ...
                                <input 
                                    ...
                                    name="slug" 
                                    value="{{ old('slug', '') }}" 
                                    placeholder="ryan-vanya (otomatis jika kosong)"
                    ...
                    <div class="pt-4 border-t border-sand-200 space-y-3">
                        ...
                        @if($themes->isEmpty())
                            <div class="p-8 rounded-2xl bg-sand-50/80 border border-dashed border-sand-300 text-center space-y-2">
                                <i data-lucide="palette" class="w-8 h-8 text-sand-400 mx-auto"></i>
                                <p class="font-bold text-xs text-charcoal-900">Belum Ada Template yang Diaktifkan</p>
                                <p class="text-[11px] text-sand-600">Super Admin belum mengaktifkan template untuk role Partner.</p>
                            </div>
                        @else
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
                        ...
                        <div class="p-4 rounded-xl bg-sand-50/80 border border-sand-200 space-y-3 text-xs">
                            ...
                            <div>
                                ...
                                <input type="text" name="groom_name" value="{{ old('groom_name', '') }}" required placeholder="Contoh: Ryan Pratama, S.Kom" class="w-full px-3 py-2 rounded-xl border border-sand-200 bg-white text-xs">
                            </div>

                            <div class="grid grid-cols-2 gap-2.5">
                                <div>
                                    <label class="font-bold text-charcoal-900 block mb-1">Nama Panggilan <span class="text-rose-500">*</span></label>
                                    <input type="text" name="groom_nickname" value="{{ old('groom_nickname', '') }}" required placeholder="Contoh: Ryan" class="w-full px-3 py-2 rounded-xl border border-sand-200 bg-white text-xs">
                                </div>
                                <div>
                        ...
                        <div class="p-4 rounded-xl bg-sand-50/80 border border-sand-200 space-y-3 text-xs">
                            ...
                            <div>
                                <label class="font-bold text-charcoal-900 block mb-1">Nama Lengkap &amp; Gelar <span class="text-rose-500">*</span></label>
                                <input type="text" name="bride_name" value="{{ old('bride_name', '') }}" required placeholder="Contoh: Vanya Maharani, S.Ked" class="w-full px-3 py-2 rounded-xl border border-sand-200 bg-white text-xs">
                            </div>

                            <div class="grid grid-cols-2 gap-2.5">
                                <div>
                                    <label class="font-bold text-charcoal-900 block mb-1">Nama Panggilan <span class="text-rose-500">*</span></label>
                                    <input type="text" name="bride_nickname" value="{{ old('bride_nickname', '') }}" required placeholder="Contoh: Vanya" class="w-full px-3 py-2 rounded-xl border border-sand-200 bg-white text-xs">
                                </div>
                                <div>
                                    <label class="font-bold text-charcoal-900 block mb-1">Urutan Anak</label>
                ...
                <div x-show="currentStep === 5" data-step="5" class="space-y-4" style="display: none;">
                    ...
                    <div class="p-4 rounded-xl bg-sand-50/80 border border-sand-200 space-y-3 text-xs">
                        ...
                        <div class="space-y-2">
                            <label class="font-bold text-charcoal-900 block">Pilih Foto-Foto Prewedding Klien (Opsional)</label>
                            <input 
                                type="file" 
                                name="gallery_files[]" 
                                multiple
                ...
                <div x-show="currentStep === 6" data-step="6" class="space-y-4" style="display: none;">
                    <div class="pb-2 border-b border-sand-200">
                        ...
                        <p class="text-[11px] text-sand-600">Atur rekening transfer amplop, alamat kado fisik, kutipan ayat, dan rilis undangan klien.</p>
                    </div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div class="p-4 rounded-xl bg-sand-50/80 border border-sand-200 space-y-2.5 text-xs">
                            ...
                            <div>
                                <label class="font-bold text-charcoal-900 block mb-1">Nama Bank</label>
                                <input type="text" name="bank_2_name" value="{{ old('bank_2_name', '') }}" placeholder="BCA / Mandiri / BRI / BSI" class="w-full px-3 py-2 rounded-xl border border-sand-200 bg-white text-xs">
                            </div>
                            <div>
                                <label class="font-bold text-charcoal-900 block mb-1">Nomor Rekening</label>
                    ...
                    <div class="p-4 rounded-xl bg-sand-50/80 border border-sand-200 space-y-2 text-xs">
                        ...
                        >{{ old('whatsapp_template', '') }}</textarea>
                    ...
                    <div class="flex items-center justify-between pt-4 border-t border-sand-200">
                        ...
                        >
                            ...
                            <span>Terbitkan Undangan Klien</span>
                        </button>
                    </div>
                </div>
