    ...
    <div class="space-y-8" x-data="{ toast: { show: false, message: '' }, showToast(msg) { this.toast.message = msg; this.toast.show = true; setTimeout(() => this.toast.show = false, 3000); } }">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <div class="flex items-center gap-2 text-xs font-bold text-charcoal-950 bg-sand-200/80 px-4 py-2.5 rounded-2xl">
                <span>Total: {{ $totalWishes }} Pesan Masuk</span>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div class="p-5 rounded-2xl glass-panel border border-sand-200 flex items-center justify-between">
                <div>
                    <span class="text-[10px] font-bold uppercase tracking-wider text-sand-500">Total Ucapan</span>
                    <div class="font-serif text-2xl font-bold text-charcoal-950">{{ $totalWishes }}</div>
                </div>
            ...
            <div class="p-5 rounded-2xl glass-panel border border-sand-200 flex items-center justify-between">
                <div>
                    <span class="text-[10px] font-bold uppercase tracking-wider text-sand-500">Disembunyikan (Spam)</span>
                    <div class="font-serif text-2xl font-bold text-amber-600">{{ $totalHidden }}</div>
                </div>
        ...
        <div class="p-4 rounded-2xl glass-panel border border-sand-200 flex flex-col sm:flex-row items-center justify-between gap-4">
            <form method="GET" action="{{ route('wishes.index') }}" class="flex-1 w-full flex flex-col sm:flex-row items-center gap-3">
